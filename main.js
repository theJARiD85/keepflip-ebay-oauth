...
function tokenEncryptionKey() {
  const key = Buffer.from(requiredEnv('EBAY_TOKEN_ENCRYPTION_KEY'), 'base64');
  if (key.length !== 32) {
    throw new Error('EBAY_TOKEN_ENCRYPTION_KEY must be base64 for exactly 32 bytes.');
  }
  return key;
}
...
function stateSecret() {
  ...
  if (Buffer.byteLength(secret, 'utf8') < 32) {
    throw new Error('EBAY_OAUTH_STATE_SECRET must be at least 32 bytes long.');
  }
  return secret;
}
...
function loadConfig(environment) {
  ...
  const prefix = normalized === 'sandbox' ? 'EBAY_SANDBOX' : 'EBAY_PRODUCTION';
...
function createAdminDatabases(req) {
  ...
  if (!apiKey) {
    throw new Error('Appwrite did not provide the function dynamic API key.');
  }
...
function authenticatedUserId(req) {
  ...
  if (!userId) {
    throw new HttpError(401, 'You must be signed in to KeepFlip to connect eBay.');
  }
  return userId;
}
...
function createState(userId, environment, secret) {
  return encodeState(
    {
      v: 1,
      userId,
...
function verifyState(state, expectedEnvironment, secret) {
  if (typeof state !== 'string' || !state.includes('.')) {
    throw new HttpError(400, 'The eBay OAuth state is missing or invalid.');
  }

  const [encodedPayload, suppliedSignature, ...extra] = state.split('.');
  if (!encodedPayload || !suppliedSignature || extra.length > 0) {
    throw new HttpError(400, 'The eBay OAuth state is invalid.');
  }
  ...
  if (supplied.length !== expected.length || !timingSafeEqual(supplied, expected)) {
    throw new HttpError(400, 'The eBay OAuth state signature is invalid.');
  }
  ...
  try {
    ...
    throw new HttpError(400, 'The eBay OAuth state payload is invalid.');
  }
  ...
  if (
    ...
    throw new HttpError(400, 'The eBay OAuth state has expired or does not match this callback.');
  }

  return payload;
}

function connectionDocumentId(userId, environment) {
  return createHash('sha256')
    .update(`${userId}:${environment}`, 'utf8')
    .digest('base64url')
    .slice(0, 36);
}
...
function decryptSecret(value, key) {
  ...
  if (version !== 'v1' || !ivText || !tagText || !ciphertextText || extra.length > 0) {
    throw new Error('Stored eBay token ciphertext is invalid.');
  }
...
function expiryIso(seconds, fieldName) {
...
async function postTokenRequest(config, form, operation) {
  ...
  if (!payload?.access_token || !payload?.expires_in) {
    throw new Error(`eBay OAuth ${operation} did not return a usable access token.`);
  }
...
async function saveNewConnection({ databases, config, userId, tokenResponse }) {
  ...
  const documentId = connectionDocumentId(userId, config.environment);
  const accessTokenExpiresAt = expiryIso(
  ...
  await databases.upsertDocument({
    ...
    data: {
      ...
      accessTokenCiphertext: encryptSecret(tokenResponse.access_token, config.encryptionKey),
      accessTokenExpiresAt,
      refreshTokenCiphertext: encryptSecret(tokenResponse.refresh_token, config.encryptionKey),
      refreshTokenExpiresAt: expiryIso(
        tokenResponse.refresh_token_expires_in,
        'refresh token expiry',
      ),
      scopeList: config.scopes.join(' '),
      tokenType: String(tokenResponse.token_type || 'User Access Token'),
      isActive: true,
      connectedAt: now,
      updatedAt: now,
    },
...
async function getConnection(databases, config, userId) {
  const documentId = connectionDocumentId(userId, config.environment);
  try {
    ...
    if (Number(caught?.code) === 404) return null;
    throw caught;
  }
}

async function refreshStoredConnection({ databases, config, connection }) {
  const refreshExpiry = Date.parse(connection.refreshTokenExpiresAt || '');
  if (!Number.isFinite(refreshExpiry) || refreshExpiry <= Date.now()) {
    throw new HttpError(401, 'Your eBay authorization has expired. Reconnect your eBay account.');
  }

  const refreshToken = decryptSecret(connection.refreshTokenCiphertext, config.encryptionKey);
  const tokenResponse = await exchangeRefreshToken(config, refreshToken);
  const now = new Date().toISOString();
  const accessTokenExpiresAt = expiryIso(
    tokenResponse.expires_in,
    'access token expiry',
  ...
  await databases.updateDocument({
    ...
    data: {
      accessTokenCiphertext: encryptSecret(tokenResponse.access_token, config.encryptionKey),
      accessTokenExpiresAt,
      tokenType: String(tokenResponse.token_type || connection.tokenType || 'User Access Token'),
      isActive: true,
      updatedAt: now,
    },
  });
...
function appReturnUrl(config, status) {
  const url = new URL(config.appReturnUrl);
...
async function handleConnect({ req, res }) {
  ...
  const environment = normalizeEnvironment(body.environment, 'OAuth environment');
  const config = loadConfig(environment);
  const state = createState(userId, environment, config.stateSecret);
  const ebay = createEbayClient(config);

  const authorizationUrl = ebay.generateUserAuthorizationUrl(
...
async function handleStatus({ req, res, databases }) {
  const userId = authenticatedUserId(req);
  const body = requestBody(req);
  const environment = normalizeEnvironment(body.environment, 'OAuth environment');
  const config = loadConfig(environment);
  const connection = await getConnection(databases, config, userId);
  ...
  const accessExpiry = Date.parse(connection.accessTokenExpiresAt || '');
  const refreshExpiry = Date.parse(connection.refreshTokenExpiresAt || '');
  const refreshExpired = !Number.isFinite(refreshExpiry) || refreshExpiry <= Date.now();
  ...
  return res.json({
    ...
    accessTokenExpired: !Number.isFinite(accessExpiry) || accessExpiry <= Date.now(),
...
async function handleRefresh({ req, res, databases }) {
  ...
  const refreshed = await refreshStoredConnection({
    databases,
    config,
    connection,
  });
...
async function handleCallback({ req, res, log, error, databases, environment }) {
  const config = loadConfig(environment);
  const authorizationCode = queryValue(req, 'code');
  const returnedState = queryValue(req, 'state');
  const providerError = queryValue(req, 'error');
  const isAuthSuccessful = queryValue(req, 'isAuthSuccessful');

  if (providerError || isAuthSuccessful === 'false' || !authorizationCode) {
    log(`eBay ${environment} authorization was declined or returned without a code.`);
    return res.redirect(appReturnUrl(config, 'declined'), 302);
  }

  try {
    const state = verifyState(returnedState, environment, config.stateSecret);
    const tokenResponse = await exchangeAuthorizationCode(config, authorizationCode);
    ...
    log(`eBay ${environment} account connected for KeepFlip user ${state.userId}.`);
    return res.redirect(appReturnUrl(config, 'connected'), 302);
  } catch (caught) {
    error(`eBay ${environment} OAuth callback failed: ${caught?.message || String(caught)}`);
    return res.redirect(appReturnUrl(config, 'error'), 302);
  }
}
...
export default async function main({ req, res, log, error }) {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/status') {
      return await handleStatus({ req, res, databases });
    }

    if (req.method === 'POST' && path === '/refresh') {
      return await handleRefresh({ req, res, databases });
    }

    if (req.method === 'GET' && path === SANDBOX_CALLBACK_PATH) {
    ...
    if (req.method === 'GET' && path === PRODUCTION_CALLBACK_PATH) {
      return await handleCallback({
        req,
        res,
        log,
